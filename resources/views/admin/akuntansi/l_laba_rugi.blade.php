          ...
          <table class="table mb-5 text-nowrap" width="100%">
            ...
            {{-- <table class="table table-bordered">
              ...
              <tbody>
                ...
                @foreach ($trade as $r)
                  @if ($r->akun->kode_akun >= 6000 && $r->akun->kode_akun <= 6999)
                    @foreach ($transaksi->where('kode_akun',$r->kode_akun) as $k)
                      @php $kredit += $r->debet @endphp
                    @endforeach
                      <tr>
                        <td>{{$r->akun->kode_akun}}</td>
                        <td>{{$r->where('kode_akun',$k->kode_akun)->sum('debet')}}</td>
                      </tr>
                  @endif
                @endforeach
