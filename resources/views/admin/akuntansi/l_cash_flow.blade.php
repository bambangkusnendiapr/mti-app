    ...
    <div class="container">
        ...
        <div class="row ">
          ...
          @php
            ...
            $kredit = 0;
          @endphp
          @foreach ($kode_akun->whereBetween('kode_akun',[999,1999]) as $ka)
            @foreach ($ka->transaksi as $pendapatan)
                @php
                    $totalkredit = $totalkredit + $pendapatan->kredit;
                    $totaldebet = $totaldebet + $pendapatan->debet;
                @endphp
            @endforeach

            <tbody>
                @php
                $debet  = $ka->transaksi->sum('debet');
                $kredit = $ka->transaksi->sum('kredit');
                @endphp
